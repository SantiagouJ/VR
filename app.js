...
function createAmbientMusic() {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    const pad1 =
      Math.sin(2 * Math.PI * 110 * t + Math.sin(2 * Math.PI * 0.1 * t) * 2) *
      0.04;
    const pad2 =
      Math.sin(2 * Math.PI * 165 * t + Math.sin(2 * Math.PI * 0.08 * t) * 2) *
      0.03;
    const pad3 =
      Math.sin(2 * Math.PI * 220 * t + Math.sin(2 * Math.PI * 0.12 * t) * 2) *
      0.02;
...
const GROUP_KEYWORDS = [
  ...
  {
    keywords: ["ventana", "window", "vidrio", "glass"],
    label: "Ventanas",
    icon: "window",
  },
  { keywords: ["piso", "suelo", "floor"], label: "Pisos", icon: "floor" },
  {
    keywords: ["techo", "ceiling", "cielo", "cielorraso"],
    label: "Techos",
    icon: "ceiling",
  },
  {
    keywords: ["cocina", "kitchen", "mesón", "meson", "encimera"],
    label: "Cocina",
    icon: "kitchen",
  },
  {
    keywords: ["baño", "bano", "bathroom", "sanitario", "lavamanos"],
    label: "Baño",
    icon: "bath",
  },
  {
    keywords: ["escalera", "stair", "escalón", "escalon"],
    label: "Escaleras",
    icon: "stairs",
  },
  {
    keywords: ["columna", "column", "pilar", "viga"],
    label: "Estructura",
    icon: "pillar",
  },
  {
    keywords: ["balcon", "terraza", "balcony", "terrasse"],
    label: "Exterior",
    icon: "balcony",
  },
  {
  ...
  {
    keywords: ["marco", "moldura", "zocalo", "zócalo", "baseboard", "trim"],
    label: "Molduras",
    icon: "trim",
  },
];
...
function getFilteredCategories() {
  ...
  const allowedCategories =
    SURFACE_MATERIALS[surfaceType] || SURFACE_MATERIALS["default"];
...
function createNoise2D(rand) {
  const S = 256,
    perm = new Uint8Array(S * 2);
  const gx = new Float32Array(S),
    gy = new Float32Array(S);
  for (let i = 0; i < S; i++) {
...
function drawMarble(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const v1 =
        Math.pow(1 - Math.abs(Math.sin(vn1 * Math.PI * 2 + wwx * 6)), 8) * 0.6;
      const vn2 = fbm(noise, wwx * 10 + 5.2, wwy * 10 + 1.3, 4);
      const v2 =
        Math.pow(1 - Math.abs(Math.sin(vn2 * Math.PI * 3 + wwy * 12)), 12) *
        0.35;
...
function drawWood(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const ring =
        Math.sin(
          (wy * ringScale + fbm(noise, nx * 2 + ox, ny * 2 + oy, 3) * 1.5) *
            Math.PI *
            2,
        ) *
          0.5 +
      ...
      const grain =
        Math.pow(
          Math.abs(
            Math.sin(wy * 80 + fbm(noise, nx * 40 + ox, ny * 2 + oy, 2) * 4),
          ),
          0.3,
        ) * 0.15;
      const ray =
        Math.pow(Math.max(0, noise(nx * 2 + ox + 30, ny * 30 + oy + 30)), 4) *
        0.08;
      ...
      if (hasKnot) {
        ...
        if (d < knotR * 3) {
          const kr = Math.sin(
            (d / knotR) * Math.PI * 4 + noise(nx * 20, ny * 20) * 2,
          );
      ...
      const t = Math.max(
        0,
        Math.min(1, ring * 0.5 + grain + ray + lenV + knot),
      );
      const pn = (rand() - 0.5) * 3;
      const i = (py * s + px) * 4;
...
function drawStone(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const layer =
        Math.sin(ny * 15 + fbm(noise, nx * 4, ny * 4, 3) * 3) * 0.05;
      const crystal =
        Math.pow(Math.max(0, noise(nx * 25 + ox + 50, ny * 25 + oy + 50)), 6) *
        50;
...
function drawConcrete(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const sf =
        turbulence(noise, nx * 15 + ox + 20, ny * 15 + oy + 20, 4) * 0.05;
      ...
      for (const p of pinholes) {
        const dx = nx - p.x,
          ...
          d2 = dx * dx + dy * dy;
        if (d2 < p.r * p.r)
          pin = Math.max(pin, (1 - Math.sqrt(d2) / p.r) * 0.25);
      }
      let aggV = 0;
      for (const a of aggs) {
        const dx = nx - a.x,
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      if (isMortar) {
        ...
        const colorVar =
          (((brickSeed * 16807 + 7) % 2147483647) / 2147483647) * 30 - 15;
        const surf = fbm(noise, nx * 12 + ox, ny * 12 + oy, 4) * 25;
        const chip =
          Math.pow(
            Math.max(0, noise(nx * 30 + ox + 50, ny * 30 + oy + 50)),
            6,
          ) * 40;
        const pn = (rand() - 0.5) * 6;
        data[i] = clamp255(br + colorVar + surf + chip + pn);
        data[i + 1] = clamp255(
          bg + colorVar * 0.7 + surf * 0.8 + chip * 0.6 + pn,
        );
        data[i + 2] = clamp255(
          bb + colorVar * 0.5 + surf * 0.6 + chip * 0.4 + pn,
        );
      }
      data[i + 3] = 255;
    }
...
function drawMetal(data, s, base, accent, rand, noise) {
  ...
  const ca = Math.cos(brushAngle),
    sa = Math.sin(brushAngle);
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const brush =
        Math.sin(ry * 200 + noise(rx * 3 + ox, ry * 0.5 + oy) * 8) * 0.12;
      const scratch1 =
        Math.pow(
          Math.max(0, Math.abs(noise(nx * 40 + ox + 10, ny * 2 + oy + 10))),
          8,
        ) * 0.15;
      const scratch2 =
        Math.pow(
          Math.max(0, Math.abs(noise(nx * 2 + ox + 30, ny * 40 + oy + 30))),
          8,
        ) * 0.1;
      const lg = fbm(noise, nx * 2 + ox + 50, ny * 2 + oy + 50, 3) * 0.08;
      const spec =
        Math.pow(Math.max(0, noise(nx * 15 + ox + 70, ny * 15 + oy + 70)), 4) *
        30;

      const t = Math.max(
        0,
        Math.min(1, 0.5 + brush + scratch1 + scratch2 + lg),
      );
      const pn = (rand() - 0.5) * 3;
      const i = (py * s + px) * 4;
...
function drawFabric(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const weave =
        (Math.floor(nx * weaveScale) + Math.floor(ny * weaveScale)) % 2 === 0
          ? warpThread * 0.15
          : weftThread * 0.15;
...
function drawLeather(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const pore =
        Math.pow(Math.max(0, noise(nx * 40 + ox + 10, ny * 40 + oy + 10)), 3) *
        0.15;
      const crease =
        Math.pow(
          1 -
      ...
      const spec =
        Math.pow(Math.max(0, noise(nx * 8 + ox + 60, ny * 8 + oy + 60)), 3) *
        20;
...
function drawPlaster(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const surf =
        turbulence(noise, nx * 12 + ox + 10, ny * 12 + oy + 10, 3) * 0.08;
      const crack =
        Math.pow(
          1 -
            Math.abs(
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      if (isGrout) {
        ...
        const colorVar =
          (((tileSeed * 16807 + 7) % 2147483647) / 2147483647) * 8 - 4;
        const glaze = fbm(noise, nx * 8 + ox + 10, ny * 8 + oy + 10, 3) * 10;
        const spec =
          Math.pow(Math.max(0, noise(nx * 4 + ox + 30, ny * 4 + oy + 30)), 2) *
          15;
        const pn = (rand() - 0.5) * 2;
...
function drawRust(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const lg = fbm(noise, nx * 3 + ox, ny * 3 + oy, 4) * 0.3;
      const pitting =
        turbulence(noise, nx * 15 + ox + 10, ny * 15 + oy + 10, 4) * 0.2;
      const flake =
        Math.pow(turbulence(noise, nx * 8 + ox + 20, ny * 8 + oy + 20, 3), 2) *
        0.15;
      const streak =
        Math.pow(
          Math.max(
      ...
      const orangeShift =
        turbulence(noise, nx * 6 + ox + 60, ny * 6 + oy + 60, 3) * 30;
      const pn = (rand() - 0.5) * 5;
      const i = (py * s + px) * 4;
...
function drawAsphalt(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const grit =
        turbulence(noise, nx * 20 + ox + 10, ny * 20 + oy + 10, 4) * 0.12;
...
function drawGlass(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const frost =
        fbm(noise, (nx + warp) * 8 + ox + 10, (ny + warp) * 8 + oy + 10, 4) *
        0.06;
      const spec =
        Math.pow(Math.max(0, noise(nx * 4 + ox + 30, ny * 4 + oy + 30)), 2) *
        25;
      const edge =
        Math.pow(Math.max(0, noise(nx * 2 + ox + 50, ny * 2 + oy + 50)), 3) *
        15;
      ...
      data[i + 1] = clamp255(
        bg + (ag - bg) * t + spec * 0.95 + edge * 0.9 + pn,
      );
      data[i + 2] = clamp255(
        bb + (ab - bb) * t + spec * 0.9 + edge * 0.85 + pn,
      );
      data[i + 3] = 255;
    }
  }
...
function drawMoss(data, s, base, accent, rand, noise) {
  ...
  for (let py = 0; py < s; py++) {
    for (let px = 0; px < s; px++) {
      ...
      const detail =
        turbulence(noise, nx * 15 + ox + 10, ny * 15 + oy + 10, 4) * 0.15;
      const fiber = Math.abs(noise(nx * 40 + ox + 20, ny * 40 + oy + 20)) * 0.1;
      const depth = fbm(noise, nx * 3 + ox + 40, ny * 3 + oy + 40, 3) * 20;
      const yellowVar =
        Math.max(0, noise(nx * 6 + ox + 60, ny * 6 + oy + 60)) * 15;

      const t = Math.max(0, Math.min(1, 0.4 + clumps + detail + fiber));
      const pn = (rand() - 0.5) * 5;
      const i = (py * s + px) * 4;
      data[i] = clamp255(
        br + (ar - br) * t - depth * 0.3 + yellowVar * 0.5 + pn,
      );
      data[i + 1] = clamp255(
        bg + (ag - bg) * t + depth * 0.2 + yellowVar * 0.3 + pn,
      );
...
function getTileTexture(tile) {
  ...
  const nCanvas = generateNormalFromCanvas(
    canvas,
    NORMAL_STRENGTH[tile.type] || 1.0,
  );
...
const panelSubtitleEl = document.querySelector(
  "#controls .panel-header .subtitle",
);
...
const xrControllers = [0, 1].map((i) => {
  ...
  ctrl.addEventListener("connected", (e) => {
    ray.visible =
      e.data.targetRayMode === "tracked-pointer" || e.data.hand != null;
...
const VR_TUTORIAL_SCALE_Y =
  VR_TUTORIAL_SCALE_X * (VR_TUTORIAL_H / VR_TUTORIAL_W);
...
function renderVRTutorial() {
  ...
  const controls = [
    {
      icon: "🎯",
      title: "Trigger (Gatillo)",
      desc: "Seleccionar superficies y elementos del menú",
    },
    {
      icon: "✊",
      title: "Grip (Agarre)",
      desc: "Sostener y mover el panel de materiales",
    },
    {
      icon: "🕹️",
      title: "Stick Izquierdo",
      desc: "Moverte por el apartamento (con colisión con paredes)",
    },
    {
      icon: "🔄",
      title: "Stick Derecho",
      desc: "Girar la cámara (rotación suave)",
    },
    {
      icon: "📋",
      title: "Panel de acabados",
      desc: "Aparece en la mano izquierda al seleccionar una superficie",
    },
    {
      icon: "👆",
      title: "Apuntar",
      desc: "Apunta a superficies para ver el rayo láser",
    },
  ];
...
function checkVRCollision(proposedOffset) {
  ...
  function tryAxisOnly() {
    ...
    if (ax > 1e-6) {
      ...
      const hx = castVerticalWallCapsule(
        _collisionDirection,
        ax + VR_PLAYER_SKIN,
      );
    ...
    if (az > 1e-6) {
      ...
      const hz = castVerticalWallCapsule(
        _collisionDirection,
        az + VR_PLAYER_SKIN,
      );
  ...
  const hit = castVerticalWallCapsule(
    _collisionDirection,
    stepLen + VR_PLAYER_SKIN,
  );
  ...
  if (!getHitWallNormal(hit, _collisionDirection, _vrWallNormal))
    return proposedOffset;
  ...
  const slideHit = castVerticalWallCapsule(
    _vrSlideDir,
    stepLen + VR_PLAYER_SKIN,
  );
...
function renderVRPanel() {
  ...
  if (!filteredCatalog.find((c) => c.cat === vrPanelActiveCategory)) {
    vrPanelActiveCategory =
      filteredCatalog.length > 0 ? filteredCatalog[0].cat : CATALOG[0].cat;
  }
  ...
  filteredCatalog.forEach((cat) => {
    ...
    vrPanelRegions.categories.push({
      x: cx,
      y: 100,
      w: tw,
      h: 32,
  ...
  const tiles =
    filteredCatalog.find((c) => c.cat === vrPanelActiveCategory)?.tiles || [];
  const cols = 3,
    tileSize = 140,
    gap = 22,
    startX = 30,
  ...
  const selLabel = selectedGroup
    ...
    : selectedMesh
      ? formatName(selectedMesh.name || "Superficie")
      : "";
  if (selLabel) {
    ctx.fillStyle = "rgba(255,255,255,0.35)";
    ctx.font = "14px Inter, system-ui, sans-serif";
...
function getXRControllerEntry(handedness) {
  return (
    xrControllers.find(({ ctrl }) => ctrl.userData.handedness === handedness) ||
    null
  );
}
...
function updateVRDrag() {
  if (!vrDragState.active || !vrDragState.mesh || !vrDragState.controller)
    return;
  ...
  m.decompose(
    vrDragState.mesh.position,
    vrDragState.mesh.quaternion,
    vrDragState.mesh.scale,
  );
}
  ...
  for (const { ctrl } of xrControllers) {
    ...
    if (vrTutorialVisible) {
      ...
      if (tutorialHits.length > 0) {
        ...
        vrPointerDot.position.addScaledVector(
          vrTutorialMesh.getWorldDirection(new THREE.Vector3()),
          0.002,
        );
    ...
    if (vrPanelVisible) {
      ...
      if (hits.length > 0) {
        ...
        vrPointerDot.position.addScaledVector(
          vrPanelMesh.getWorldDirection(new THREE.Vector3()),
          0.002,
        );
        vrPointerDot.visible = true;
        return;
...
function commitXRReferenceSpace() {
  ...
  // Place the tracking origin so the player's virtual feet land at xrPlayerPos.
  // Formula: P = -R(xrYaw) * xrPlayerPos  (assumes player near tracking origin).
  // Avoids a second getViewerPose call in the same frame, which causes
  // inconsistent poses on Meta Quest 3 when both sticks are active.
  ...
  renderer.xr.setReferenceSpace(
    xrBaseReferenceSpace.getOffsetReferenceSpace(offsetTransform),
  );
}
...
function updateXRLocomotion(now, frame) {
  ...
  const dt = xrLastFrameTime
    ? Math.min(0.1, (now - xrLastFrameTime) / 1000)
    : 0;
  xrLastFrameTime = now;
  if (dt === 0) return;
  ...
  // Single getViewerPose call per frame — from the current offset space so the
  // orientation already includes our accumulated xrYaw (correct for movement direction).
  const viewerPose = frame.getViewerPose(renderer.xr.getReferenceSpace());
  if (!viewerPose) return;

  if (
    Math.abs(moveX) > XR_STICK_DEADZONE ||
    Math.abs(moveZ) > XR_STICK_DEADZONE
  ) {
    const dir = getXRMoveDirectionWorld(
      moveX,
      moveZ,
...
const highlightEdges = new THREE.LineBasicMaterial({
  color: 0x64b5f6,
  linewidth: 2,
});
...
function filterEditableMeshIndices(indices) {
  return indices.filter(
    (i) => meshParts[i] && !isMeshFinishesLocked(meshParts[i]),
  );
}
...
function updateFinishesLockedUI() {
  ...
  const partialSketchfab =
    !fullFileLocked && sceneHasSketchfabHierarchyMeshes();
...
function loadFile(file) {
  ...
  gltfLoader.load(
    ...
    (error) => {
      ...
      alert(
        "Error al cargar el modelo. Asegúrate de que sea un archivo glTF/GLB válido.",
      );
    },
  );
}
...
function buildControlsUI(filename) {
  ...
  document.getElementById("part-count").textContent =
    ...
    (groupCount > 0
      ? ` en ${groupCount} grupo${groupCount !== 1 ? "s" : ""}`
      : "");
...
btnBack.addEventListener("click", () => {
  ...
  clearHighlight();
  partsList
    .querySelectorAll(".color-group.selected")
    .forEach((el) => el.classList.remove("selected"));
  partsList
    .querySelectorAll(".group-header.selected")
    .forEach((el) => el.classList.remove("selected"));
  selectedMesh = null;
  selectedIndex = -1;
  selectedGroup = null;
...
function findGroupForMesh(meshIndex) {
  return (
    meshGroups.find(
      (g) => g.name !== null && g.meshIndices.includes(meshIndex),
    ) || null
  );
}
...
function selectMeshAndShowFinishes(mesh, index) {
  ...
  const itemEl = partsList.querySelector(
    `.color-group[data-mesh-index="${index}"]`,
  );
...
function selectGroupAndShowFinishes(group) {
  ...
  const headerEl = partsList.querySelector(
    `.part-group[data-group-index="${gi}"] .group-header`,
  );
  ...
  const currentColor = selectedMesh?.material?.color
...
function updateFilteredCatalogUI() {
  ...
  filteredCatalog.forEach((cat) => {
    ...
    chip.className =
      "category-chip" + (cat.cat === activeCategory ? " active" : "");
...
function highlightAppliedTile() {
  document
    ...
    .forEach((el) => el.classList.remove("applied"));
  ...
  if (finish?.type === "tile") {
    const el = document.querySelector(
      `.tile-item[data-tile-id="${finish.tile.id}"]`,
    );
...
function applyTileFinish(tile) {
  ...
  indices.forEach((i) => {
    ...
    if ("roughness" in mesh.material)
      mesh.material.roughness = ROUGHNESS_MAP[tile.type] ?? 0.5;
    if ("metalness" in mesh.material)
      mesh.material.metalness = metalTypes[tile.type] ?? 0.0;
...
function updatePartListItemByIndex(meshIndex, label, bgImage, bgColor) {
  const el = partsList.querySelector(
    `.color-group[data-mesh-index="${meshIndex}"]`,
  );
  if (!el) return;
...
function highlightUploadedTexture(activeURL) {
  document
    .querySelectorAll(".uploaded-tex-item")
    .forEach((el) => el.classList.remove("applied"));
  const indices = getSelectedMeshIndices();
...
document.getElementById("btn-reset-colors").addEventListener("click", () => {
  meshParts.forEach((mesh, i) => {
    ...
    if (orig) {
      ...
      if ("normalMap" in mesh.material)
        mesh.material.normalMap = orig.normalMap || null;
      if ("roughness" in mesh.material)
        mesh.material.roughness = orig.roughness ?? 0.5;
      if ("metalness" in mesh.material)
        mesh.material.metalness = orig.metalness ?? 0;
      mesh.material.needsUpdate = true;
    }
    appliedFinishes.delete(mesh.uuid);
    updatePartListItemByIndex(
      i,
      "Original",
      null,
      mesh.material?.color
...
function animate(time, frame) {
  ...
  if (isInVR) {
    ...
    if (
      vrPanelVisible &&
      !vrDragState.active &&
      !vrPanelMesh.userData.userMoved
    ) {
...
function loadDefaultModel() {
  ...
  gltfLoader.load(
    ...
    (error) => {
      ...
      alert(
        'No se pudo cargar el modelo de ejemplo. Asegúrate de que el archivo "' +
          DEFAULT_MODEL +
...
document
  ...
  .addEventListener("click", loadDefaultModel);
